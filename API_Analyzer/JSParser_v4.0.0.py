...
def printAPI(unsafe,apiInFunc,path):
    ...
    for i in apiInFunc:
        ...
        for j in unsafe:
            if i[0][1] == j:
                print("Line "+str(i[0][0])+": "+i[0][2]+"\n")
                print("In function: "+i[1][2]+"\n")
                ''' This seems like too much information
                for ele in i[1][3]:
                    print(ele+"\n")
                '''
def printVariable(variableList,path):
    ...
    if len(variableList) > 0:
        print("******************************* Variable in unsafe API******************************\n")
        for i in variableList:
            print("Line "+str(i[0])+": "+i[2]+"\n")
def main(path):
    unsafe = [sys.argv[2]] # Receive unsafe API list from bash script
